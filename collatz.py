#!/usr/bin/env python3
# Collatz Matrix Rain — Pygame (full-digit blocks per Collatz step)
...
class MatrixColumn:
    ...
    def start_new_sequence(self, now_ms: int):
        ...
        self.row_accum = 0.0
        self.rows_until_next_block = 0  # emit first block immediately
        self.char_buffer.clear()
        self.active = True
        self.sequence_finished = False
    ...
    def _emit_block_from_value(self, n: int):
        digits = list(str(n))
        # Insert digits so they appear as a contiguous vertical block from the head upward
        for ch in digits:
            self.char_buffer.insert(0, ch)
        # Limit buffer size to screen rows + tail slack
    ...
    def update(self, dt: float, now_ms: int):
        ...
        # When the top-most glyph of the entire stream is below the screen, schedule next spawn
...
def main():
    parser = argparse.ArgumentParser(description="Collatz Matrix Rain (full-digit blocks)")
    ...
    parser.add_argument("--tail", type=int, default=30, help="Trail length in rows")
    ...
    parser.add_argument("--spawn-min-ms", type=int, default=600)
    parser.add_argument("--spawn-max-ms", type=int, default=4000)
    parser.add_argument("--bottom-fade-px", type=int, default=120)
    parser.add_argument("--trail-alpha", type=int, default=30, help="Screen fade alpha per frame (0..255)")
    parser.add_argument("--block-gap-rows", type=int, default=1, help="Blank rows between successive values")
